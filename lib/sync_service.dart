...
const defaultWebDavBackupPath = '/BaizhanSkill/backup.json';
...
class SyncSettingsStore {
  ...
  static const _lastSyncedDataKey = 'webdav.lastSyncedData';
  ...
  Future<String?> loadLastSyncedData() async {
    final preferences = await SharedPreferences.getInstance();
    return preferences.getString(_lastSyncedDataKey);
  }

  Future<void> saveLastSyncedData(String data) async {
    final preferences = await SharedPreferences.getInstance();
    await preferences.setString(_lastSyncedDataKey, data);
  }

  Future<void> clearSyncCursor() async {
    final preferences = await SharedPreferences.getInstance();
    await Future.wait([
      preferences.remove(_lastSyncAtKey),
      preferences.remove(_currentBackupPathKey),
      preferences.remove(_lastSyncedDataKey)
    ]);
  }
}
...
class WebDavSyncService {
  ...
  Future<void> testConnection(SyncConfig config) async {
    if (!config.isValid) throw const FormatException('请先填写完整的 WebDAV 配置');
  ...
  Future<List<RemoteBackup>> listBackups(SyncConfig config) async {
    ...
    final backups = files
        ...
        .toList();
    backups.sort((a, b) {
      final aTime = backupTime(a);
      final bTime = backupTime(b);
  ...
  Future<String> downloadBackup(SyncConfig config, RemoteBackup backup) async {
    if (!config.isValid) throw const FormatException('请先填写完整的 WebDAV 配置');
    return utf8.decode(await _client(config).read(backup.path));
  }
  ...
  String _timestamp(DateTime value) {
    ...
    final time =
        '${value.hour.toString().padLeft(2, '0')}${value.minute.toString().padLeft(2, '0')}${value.second.toString().padLeft(2, '0')}${value.millisecond.toString().padLeft(3, '0')}${value.microsecond.toString().padLeft(6, '0')}';
    return '${date}_$time';
  }
  ...
  DateTime? _timeFromName(String name) {
    ...
    return DateTime.tryParse(
        '${date.substring(0, 4)}-${date.substring(4, 6)}-${date.substring(6, 8)}T${time.substring(0, 2)}:${time.substring(2, 4)}:${time.substring(4, 6)}.${time.substring(6, 9)}');
  }
}
